...
"function"==typeof e&&e.call(void 0,b,d,f)})};a.setBehavior=function(){switch(a.settings.behavior){case "modal":a.settings.box.addClass("is-modal");break;default:a.settings.box.find(".lightbox-wrapper").click(function(b){if(b.target===this)return a.close(),!1})}};a.setView=function(){a.settings.title.length&&a.settings.box.find(".lightbox-header").html(a.settings.title);a.settings.footer.length&&a.settings.box.find(".lightbox-footer").html(a.settings.footer);switch(a.settings.type){case "inline":if(c(a.settings.content).length){var b=
c(a.settings.content).clone(!0);a.settings.box.find(".lightbox-content").html(b)}else a.settings.box.find(".lightbox-content").html(a.settings.content);break;case "image":a.settings.box.addClass("is-image");a.settings.box.find(".lightbox-header").hide();a.settings.box.find(".lightbox-footer").hide();a.settings.box.find(".lightbox-content").addClass("loading");a.loadImage(a.settings.href);break;case "gallery":a.gallery={items:[],loaded:[],current:0};a.settings.box.addClass("is-image");a.settings.box.addClass("is-gallery");
a.settings.box.find(".lightbox-header").hide();a.settings.box.find(".lightbox-footer").hide();a.settings.box.find(".lightbox-content").addClass("loading").html("");a.settings.box.find(".lightbox-prev").hide();a.settings.box.find(".lightbox-next").hide();c.each(c("a",c(a.settings.gallery)),function(){var b=c(this);b.attr("href").length&&-1!=c.inArray(b.attr("href").split(".").pop(),a.settings.imageTypes)&&a.gallery.items.push(b.attr("href"))});for(b in a.gallery.items)if(a.settings.href==a.gallery.items[b]){a.gallery.current=
parseInt(b);break}a.loadImage(a.gallery.current);1<a.gallery.items.length&&(a.settings.box.find(".lightbox-prev").show(),a.settings.box.find(".lightbox-next").show());a.settings.box.find(".lightbox-prev a").off("click").click(function(){--a.gallery.current;-1==a.gallery.current&&(a.gallery.current=a.gallery.items.length-1);a.loadImage(a.gallery.current);return!1});a.settings.box.find(".lightbox-next a").off("click").click(function(){a.gallery.current+=1;a.gallery.current>a.gallery.items.length-1&&
(a.gallery.current=0);a.loadImage(a.gallery.current);return!1});break;case "ajax":c.ajax({url:a.settings.href,success:function(b){a.settings.box.find(".lightbox-content").html(b)}});break;case "iframe":a.settings.box.find(".lightbox-content").html('<iframe src="'+a.settings.href+'" border="0"></iframe>')}};a.bind=function(b,c){b.off("click").click(function(){a.init(b,c);a.open();return!1})};a.open=function(){c("html,body").addClass("no-scroll");c("body").append(a.settings.box);a.settings.classes&&
a.settings.box.addClass(a.settings.classes);a.settings.box.fadeIn(450)};a.close=function(){c("html,body").removeClass("no-scroll");a.settings.box.fadeOut(450,function(){a.settings.box.remove()})}};c.fn.extend({lightbox:function(a){var b=a||{};return this.each(function(){var a=c(this);switch(b){default:case "bind":d.lightbox.bind(a,b);break;case "close":d.lightbox.close()}})}})})(window,jQuery);