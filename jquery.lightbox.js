		...
		lightbox: function(settings) {
			...
			return this.each(function(){
				...
				var lightbox = new function(){
					...
					_this.init = function(settings){

						_this.settings = $.extend(true, {
							id: 'lightbox_'+id,
							box: $('<div class="lightbox-main" id="lightbox_'+id+'" style="display:none"><div class="lightbox-overlay"></div><div class="lightbox-wrapper"><div class="lightbox-body"><div class="lightbox-close"><a href="#">&times;</a></div><div class="lightbox-prev" style="display:none;"><a href="#"><i></i></a></div><div class="lightbox-next" style="display:none;"><a href="#"><i></i></a></div><div class="lightbox-header cf"></div><div class="lightbox-content cf"></div><div class="lightbox-footer cf"></div></div></div></div>'),
							easing: typeof jQuery.ui != 'undefined' ? 'easeOutCubic' : false,
							texts: typeof app.data.texts != 'undefined' ? app.data.texts : {
								loading: 'Cargando...',
							...
							class: element.data('class') || false,
					...
					_this.setAction = function(){
						...
						switch(_this.settings.action){
							case 'delete':
								_this.settings.title = _this.settings.title.length ? _this.settings.title : _this.settings.texts.delete.title;
								_this.settings.content = _this.settings.description.length ? _this.settings.description : '<p>'+_this.settings.texts.delete.description_single+'</p>';
							...
							case 'delete-multiple':
								_this.settings.title = _this.settings.title.length ? _this.settings.title : _this.settings.texts.delete.title;
								_this.settings.content = _this.settings.description.length ? _this.settings.description : '<p>'+_this.settings.texts.delete.description_list+'</p>';
								_this.settings.footer = '<a data-dismiss="lightbox" href="#" class="btn btn-default">'+_this.settings.texts.cancel+'</a>';
					...
					_this.loadImage = function(image, callback){
						...
						if(_this.settings.type == 'gallery'){
							var src = _this.gallery.items[image];
						} else {
							var src = image;
						}
						...
						_this.preloadShowLoadingTimer = setTimeout(function(){
							...
							_this.settings.box.find('.lightbox-close').hide();
							_this.settings.box.find('.lightbox-content').addClass('loading').addClass('dark').html('');
							_this.settings.box.find('.lightbox-body').animate({'width': '44px', 'margin-top': (($(window).height() - 44)/2)+'px'}, 300, _this.settings.easing);
						...
						_this.preloadImage(src, function(img){
							...
							_this.settings.box.find('.lightbox-close').show();
					...
					_this.setView = function(){
						...
						switch(_this.settings.type){
							...
							case 'gallery':
								...
								_this.settings.box.find('.lightbox-body').css({width: '44px'});
								_this.settings.box.find('.lightbox-prev').hide();
								_this.settings.box.find('.lightbox-next').hide();
					...
					_this.open = function(){
						...
						if(_this.settings.class){
							$('.lightbox-main').addClass(_this.settings.class);
						}
						$('.lightbox-main').fadeIn(450);
					}

					_this.close = function(){
						$('html,body').removeClass('no-scroll');
						$('.lightbox-main').fadeOut(450, function(){
